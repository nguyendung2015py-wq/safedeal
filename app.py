...
def analyze_safedeal(text: str) -> AnalysisResult:
    ...
    risks = [
        ...
        {"cat": "object", "what": "Объект в залоге (ипотека банка)", 
         ...
         "fix": "Проводить сделку строго под контролем банка-залогодержателя: гашение долга в ходе сделки через СБР или официальный перевод долга.", "w": 15.0},
        
        {"cat": "object", "what": "Арест или запрет на рег. действия", 
         "kw": ["арест", "запрет", "ограничени", "судебный пристав"], 
         "law": "<b>ФЗ №218-ФЗ «О госрегистрации недвижимости» ст. 56.</b> Росреестр приостановит и откажет в регистрации перехода права. Сделка не может быть завершена до снятия запрета инициатором (суд, ФССП).", 
         "fix": "Срочный запрос актуальной выписки ЕГРН. Погашение долгов продавцом до сделки с обязательным получением постановления о снятии ареста.", "w": 30.0},
        
        {"cat": "object", "what": "Маткапитал и скрытые детские доли", 
         "kw": ["маткапитал", "материнск", "детск", "доли не выделены", "опек", "несовершеннолетн"], 
         "law": "<b>СК РФ ст. 60, ФЗ № 256-ФЗ (ч. 4 ст. 10).</b> Сделка без предварительного выделения долей детям ничтожна (ст. 168 ГК РФ). Риск расторжения сделки опекой или прокуратурой.", 
         "fix": "Обязательный запрос справки об остатке маткапитала из СФР. Требовать нотариального выделения долей до сделки и приказ опеки.", "w": 20.0},
        
        {"cat": "object", "what": "Свежее или спорное наследство", 
         "kw": ["наследств", "завещан", "свидетельство о праве", "умер", "вступил в наследство"], 
         "law": "<b>ГК РФ ст. 1149, ст. 1155.</b> Постановление Пленума ВС РФ № 9. Объявление внебрачных детей или лиц, не знавших о смерти, ведет к истребованию доли из незаконного владения покупателя (ст. 302 ГК РФ).", 
         "fix": "Высокий риск при наследстве до 3 лет. Требовать от продавца нотариальное обязательство о самостоятельном финансовом урегулировании претензий.", "w": 20.0},

        {"cat": "object", "what": "Приватизация и скрытые «отказники»", 
         "kw": ["приватиза", "соцнайм", "ордер", "отказник", "отказался от участия"], 
         "law": "<b>ГК РФ ст. 292, ЖК РФ ст. 31. ФЗ № 189-ФЗ (ст. 19).</b> Лица, отказавшиеся от участия в приватизации, приобретают бессрочное право пользования помещением. Выселить их через суд невозможно.", 
         "fix": "Запрос архивной выписки из домовой книги. Требовать снятия с регистрационного учета всех пользователей до подписания ДКП.", "w": 15.0},

        {"cat": "object", "what": "Договор ренты / Пожизненное содержание", 
         "kw": ["рент", "пожизненн", "иждивени", "содержание с иждивением"], 
         "law": "<b>ГК РФ ст. 599, 605.</b> Получатель ренты может заявить о существенном нарушении условий договора и потребовать возврата квартиры. Риск оспаривания сделки наследниками.", 
         "fix": "Проверка факта снятия обременения. Если рентополучатель жив — сделка только с его нотариального согласия. Изучить расписки о платежах.", "w": 20.0},

        {"cat": "object", "what": "Короткий срок владения", 
         "kw": ["частая смена", "перепродавалась", "год назад", "владеет всего", "владеет год", "менее 3 лет", "менее 5 лет"], 
         "law": "<b>ГК РФ ст. 170. ФЗ № 127-ФЗ ст. 61.2.</b> Быстрая перепродажа — маркер сокрытия мошенничества или предбанкротного вывода активов. Вероятность отката цепочки сделок.", 
         "fix": "Запрос выписки ЕГРН о переходе прав. Глубокая правовая экспертиза оснований каждой сделки за последние 3 года.", "w": 15.0},
        ...
        {"cat": "seller", "what": "Банкротство, долги и суды", 
         ...
         "fix": "Аудит продавца (и его супруга) по базам ФССП, ЕФРСБ, картотеке арбитражных дел (КАД) и ГАС «Правосудие».", "w": 25.0},
        ...
        {"cat": "seller", "what": "Справки ПНД/НД и дееспособность", 
         ...
         "law": "<b>ГК РФ ст. 177.</b> Сделка, совершенная гражданином, не способным понимать значение своих действий, оспорима.", 
         "fix": "Требовать личного получения справок из ПНД и НД продавцом. Оптимально — освидетельствование врачом-психиатром на сделке.", "w": 20.0},

        {"cat": "seller", "what": "Совместно нажитое имущество (Супруги)", 
         "kw": ["брак", "женат", "замужем", "супруг", "развод", "совместн"], 
         "law": "<b>СК РФ ст. 35 ч. 3. ГК РФ ст. 253.</b> Для продажи недвижимости, купленной в браке, требуется нотариальное согласие супруга.", 
         "fix": "Проверка штампов в паспорте и справок из ЗАГСа. Обязательное нотариальное согласие супруга (даже бывшего) или брачный договор.", "w": 15.0},

        {"cat": "seller", "what": "Продажа по доверенности", 
         "kw": ["доверенност", "поверенн", "представител", "за границей"], 
         "law": "<b>ГК РФ ст. 188.</b> Сделка ничтожна, если на момент ее совершения доверитель умер, признан недееспособным или доверенность отозвана.", 
         "fix": "Проверка доверенности по реестру ФНП прямо в день сделки. Настаивать на видеозвонке с реальным собственником.", "w": 20.0},
        ...
        {"cat": "trigger", "what": "Занижение цены в договоре (ДКП)", 
         ...
         "fix": "Указывать в ДКП полную реальную стоимость. Использовать аккредитив или эскроу-счет.", "w": 30.0},

        {"cat": "trigger", "what": "Встречная / Альтернативная сделка", 
         "kw": ["альтернатив", "встречн", "цепочк", "одновременн", "пока не найдем"], 
         "law": "<b>ГК РФ ст. 157.</b> Риск развала всей цепи сделок из-за срыва регистрации или отказа одного из участников.", 
         "fix": "Тщательная проверка ВСЕХ объектов в цепочке. Единовременная подача документов на регистрацию.", "w": 10.0},

        {"cat": "trigger", "what": "Отказ от ипотеки", 
         "kw": ["без ипотеки", "только наличны"], 
         "law": "Продавец избегает банковского андеррайтинга, так как документы не выдержат экспертизы.", 
         "fix": "Требовать проведения полного экспертного аудита сделки.", "w": 15.0},

        {"cat": "trigger", "what": "Требование задатка вслепую", 
         "kw": ["задаток до", "аванс до", "предоплата", "чтобы закрепить"], 
         "law": "<b>ГК РФ ст. 380, ст. 381.</b> Если покупатель после проверки откажется от сделки, продавец юридически вправе удержать задаток.", 
         "fix": "Подписание соглашения об авансе (с условием возврата) только ПОСЛЕ предоставления базового пакета документов.", "w": 15.0},
    ]
    ...
    critical_triggers = ["в конверте", "занижен", "меньше", "банкрот", "судебн", "наследств", "завещан", "доли не выделены", "опек", "арест", "запрет"]
...
def main():
    ...
    text = st.text_area("Текст для экспертизы", height=160, placeholder="Опишите ситуацию. Например: 'Квартира в ипотеке, доли детям не выделены. Хотят занижение цены. Продает пенсионер по доверенности...'")
