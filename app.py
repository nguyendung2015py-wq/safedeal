...
def analyze_safedeal(text: str) -> AnalysisResult:
    items = []
    risks = [
        {"cat": "trigger", "what": "Занижение цены / Притворная сделка", "kw": ["занижен", "конверт", "минимальная", "неполная", "расписк", "неотделим", "в половину", "половину ниже", "меньше в договоре", "полцены", "занизить"], "law": "ГК РФ ст. 170. Притворная сделка ничтожна. Риск возврата только суммы по ДКП.", "fix": "Указывать в ДКП строго 100% реальную стоимость объекта.", "w": 45.0},
        ...
        {"cat": "object", "what": "Незаконная перепланировка", "kw": ["перепланировк", "снесли стен", "мокрая точк", "объединили", "неузакон", "переделали", "снос"], "law": "ЖК РФ ст. 29. Риск штрафов и продажи квартиры с торгов по суду.", "fix": "Требовать узаконивания или закладывать бюджет на возврат в исходное состояние.", "w": 30.0},
        ...
        {"cat": "object", "what": "Арест или запрет на рег. действия", "kw": ["арест", "запрет", "пристав", "ограничения"], "law": "ФЗ №218-ФЗ ст. 56. Регистрация прав приостанавливается при наложении ареста.", "fix": "Погашение долгов до сделки, снятие ареста.", "w": 35.0},
        {"cat": "object", "what": "Маткапитал и детские доли", "kw": ["маткапитал", "детск", "опек", "пфр", "материнск", "материнский"], "law": "ФЗ №256-ФЗ ст. 10. Обязательное выделение долей. Сделка без них ничтожна.", "fix": "Проверка выделения долей и приказ опеки.", "w": 35.0},
        {"cat": "object", "what": "Свежее наследство", "kw": ["наследств", "завещан", "умер", "наследник", "по наследству"], "law": "ГК РФ ст. 1155. Суд может восстановить срок для других наследников.", "fix": "Нотариальное обязательство о финансовом урегулировании претензий.", "w": 25.0},
        {"cat": "object", "what": "Приватизация (отказники)", "kw": ["приватиз", "отказ", "отказник"], "law": "ФЗ №1541-1. Отказавшиеся от приватизации имеют право бессрочного проживания.", "fix": "Проверка архивной выписки и выписка лиц до сделки.", "w": 30.0},
        ...
        {"cat": "seller", "what": "Долги по ЖКХ и Капремонту", "kw": ["капремонт", "жкх", "коммунал", "задолженность по жкх"], "law": "ЖК РФ ст. 158. Долги по взносам на капитальный ремонт переходят на нового собственника.", "fix": "Запрос свежих справок об отсутствии задолженности из УК и ФКР в день сделки.", "w": 15.0},
    ...
    critical_triggers = ["в конверте", "занижен", "1 млн", "до миллиона", "банкрот", "наследств", "доли не выделены", "опек", "арест", "запрет", "отказник", "рент", "тюрьм", "арми", "в половину", "полцены"]
...
def main():
    ...
    st.markdown(f"""
        ...
        <div style="text-align: center; background-color: #f8fffc; padding: 10px; border-radius: 8px; border: 1px dashed #008a5e; margin-bottom: 25px; font-size: 0.95rem;">
            📌 <b>Лайфхак:</b> Сохраните этот сайт в закладки браузера, чтобы шпаргалка по налогам и проверке рисков всегда была под рукой на просмотрах квартир.
        </div>
    """, unsafe_allow_html=True)

    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["🛡️ АУДИТ", "💰 НАЛОГИ", "🏦 ИПОТЕКА", "📉 ОЦЕНКА", "💼 РАСХОДЫ", "📅 ЭТАПЫ", "🧠 ПСИХОЛОГИЯ"])
    ...
    with tab1:
        ...
        text = st.text_area("Текст для экспертизы", key="my_text", height=140, placeholder="Пишите как есть: маткапитал, перепланировка, сумма в договоре в половину ниже, долги по жкх...")
    ...
    with tab4:
        st.markdown("### 📉 Оценка: Цена упрямства (Для продавцов)")
        st.markdown("Рынок не прощает простоя. Эмоциональная привязанность к цене стоит реальных денег. Посчитаем упущенную выгоду.")
        ...
        if st.button("РАССЧИТАТЬ ПОТЕРИ"):
            if wish_price <= real_price:
                st.success("✅ Цена в рынке. Объект будет продан в оптимальные сроки без финансовых потерь на простое.")
            else:
                diff_percent = ((wish_price - real_price) / real_price) * 100
                overprice = wish_price - real_price
                # Доходность по вкладу (возьмем консервативные 16% годовых) от реальной цены, которую можно было бы получить
                lost_interest_month = (real_price * 0.16) / 12
                
                st.error(f"⚠️ Цена выше рынка на **{diff_percent:.1f}%** ({overprice:,.0f} ₽).".replace(',', ' '))
                st.markdown(f"""
                Квартира рискует превратиться в "музей" и продаваться месяцами. Пока вы ждете покупателя на завышенную цену, вы упускаете гарантированный доход:
                
                * Упущенная выгода по банковскому вкладу: **~ {lost_interest_month:,.0f} ₽ каждый месяц**.
                * Плюс оплата коммунальных платежей за пустующий объект.
                
                **Итог:** Стоять на своем часто выходит дороже, чем сделать разумную скидку и инвестировать полученные деньги.
                """.replace(',', ' '))
    ...
    with tab6:
        ...
        <b>💡 Важно:</b> На каждом из этих этапов цена ошибки измеряется миллионами рублей. Задача профильного специалиста — не просто собрать бумаги, а организовать процесс так, чтобы исключить риск потери денег, судов и отказов в регистрации.
        </div>
        """, unsafe_allow_html=True)

    with tab7:
        st.markdown("### 🧠 Психология переговоров: Базовые ошибки")
        st.info("Переговоры о цене и условиях — это шахматная партия. Эмоции здесь стоят дорого.")
        
        st.markdown("""
        **❌ Позиция «Я тут главный» (Частая ошибка продавцов)**
        Грубость, нежелание показывать документы до задатка или фразы в стиле «не нравится — не покупайте». Это отпугивает адекватных покупателей. В итоге объект зависает на рынке, и продавать его приходится с огромным дисконтом.
        
        **❌ Синдром «Влюбленного покупателя»**
        Прийти на просмотр всей семьей, восхищаться ремонтом, громко обсуждать, где будет стоять диван, и сразу показать, что квартира вам идеально подходит. Продавец моментально считывает вашу зависимость и не уступит ни рубля.
        
        **❌ Иллюзия идеального объекта**
        Люди часто закрывают глаза на юридические дыры (например, свежее наследство), потому что «уж очень вид из окна красивый». Это прямой путь к потере денег по суду.
        """)
        
        st.markdown("""
        <div style="background-color: #f8f9fa; padding: 15px; border-left: 4px solid #f59e0b; border-radius: 5px; margin-top: 15px;">
        <b>Мой подход:</b> Здесь описана только база. Жесткие скрипты перехвата инициативы, техники экологичного снижения цены и аргументированный торг я применяю лично, защищая интересы своих клиентов на сделках.
        </div>
        """, unsafe_allow_html=True)
